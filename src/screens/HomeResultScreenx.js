...
import {doFullSearch, resetFullSearch, updateFullSearchResultLikes} from "../redux/search/actions";
...
class HomeResultScreen extends PureComponent<Props> {
    ...
    _renderItem = ({item, index}) => {
        return <Item requireLogin={this.requireLogin} last={item.id===this.props.search.fullSearchResult.result[this.props.search.fullSearchResult.result.length-1].id} navigation={this.props.navigation} data={item} key={index}/>;
    };
    _renderHeader = () => {
        return (
                    ...
                    item.sale_up_to?
                        <View style={Styles.saleUpTo}>
                            <Text style={Styles.saleUpToText}>
                                GIẢM ĐẾN {numeral(item.sale_up_to/1000).format('0,000')}K
                            </Text>
                        </View>
                        :undefined
                }
