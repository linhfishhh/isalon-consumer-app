...
export const getResultSearchTabLatest = (extra = {}, restart = true) =>{
    return async (dispatch, getState) => {
        let tab = getState().new_search_tab_latest;
        ...
        try{
            let items;
            if(!restart){
                dispatch(updateSearchTabLatestState({
                    loading_more: true,
                }));
...
export const updateSalonNewLike = (id, liked) => {
    return (dispatch, getState) => {
        if(getState().new_search_tab_latest.result.items === null){
            return;
        }
        if(getState().new_search_tab_latest.result.items.length === 0){
            return;
        }
        dispatch(updateSearchTabLatestState({
            result: {
                ...getState().new_search_tab_latest.result,
                items: getState().new_search_tab_latest.result.items.map((salon) => {
                    if(salon.id !== id){
                        return salon;
                    }
                    return {
