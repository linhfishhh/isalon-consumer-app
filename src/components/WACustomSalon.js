...
export default class WACustomSalon extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      // data : data
      ...
      >
        ...
        >
          <View style={Styles.itemImageAndPrice}>
            ...
            {item.sale_up_to_percent_cache !== 0 ? (
              <Text style={Styles.promotion}>
                -{item.sale_up_to_percent_cache}%
              </Text>
            ) : (
              undefined
            )}
          ...
          <View style={Styles.itemContent}>
            ...
            <View style={Styles.star}>
              ...
              <Text style={Styles.itemTextPrice}>
                từ {item.price_from_cache / 1000}k
              </Text>
            </View>
          </View>
        </View>
  ...
  render() {
    ...
    ) : this.props.data.items != undefined && this.props.data.items.length > 0 ? (
      this.props.data.items.map((items, index) => {
        return (
          <View style={Styles.content} key={"salon" + index}>
            <View style={Styles.rowDanhMuc}>
              <Text style={Styles.titleCatory} numberOfLines={1}>{items.name}</Text>
              <TouchableOpacity
                onPress={() => {
                  this.props.navigation.navigate("home_result", {
                    query: {"key":"CustomSalon","id" : items.id}
                  });
                }}
              >
                <View style={{ flexDirection: "row" }}>
