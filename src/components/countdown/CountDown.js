...
const DEFAULT_DIGIT_STYLE = { backgroundColor: '#21232C' };
...
class CountDown extends React.Component {
  ...
  //   return nextProps.until !== this.props.until;
  // }
  ...
  renderDigit = (d) => {
    ...
    return (
      <View style={[
        styles.digitCont,
        digitStyle,
        { width: size * 1.8, height: size * 2.4 },
      ]}>
        <Text style={[
          styles.digitTxt,
          { fontSize: size * 1.7},
  ...
  renderDoubleDigits = (label, digits1, digits2) => {
    return (
      <View style={styles.doubleDigitCont}>
        <View style={styles.timeInnerCont}>
          {this.renderDigit(digits1)}
          {this.renderDigit(digits2)}
        </View>
  ...
  renderSeparator = () => {
    ...
    return (
      <View style={{ justifyContent: 'center', alignItems: 'center' , marginBottom: 2, marginRight: 2,}}>
        <Text style={[
          styles.separatorTxt,
          { fontSize: size * 2.2 },
          separatorStyle,
  ...
  renderCountDown = () => {
    ...
    return (
      ...
      >
        ...
        {days > 0 && timeToShow.includes('D') ? this.renderDoubleDigits(timeLabels.d, newTime[0].substring(0, 1), newTime[0].substring(1, 2)) : null}
        {/* {showSeparator && timeToShow.includes('D') && timeToShow.includes('H') ? this.renderSeparator() : null} */}
        {days > 0 && this.renderSeparator()}
        {timeToShow.includes('H') ? this.renderDoubleDigits(timeLabels.h, newTime[1].substring(0, 1), newTime[1].substring(1, 2)) : null}
        {/* {showSeparator && timeToShow.includes('H') && timeToShow.includes('M') ? this.renderSeparator() : null} */}
        {this.renderSeparator()}
        {timeToShow.includes('M') ? this.renderDoubleDigits(timeLabels.m, newTime[2].substring(0, 1), newTime[2].substring(1, 2)) : null}
        {/* {showSeparator && timeToShow.includes('M') && timeToShow.includes('S') ? this.renderSeparator() : null} */}
        {this.renderSeparator()}
        {timeToShow.includes('S') ? this.renderDoubleDigits(timeLabels.s, newTime[3].substring(0, 1), newTime[3].substring(1, 2)) : null}
      </Component>
    );
  };
